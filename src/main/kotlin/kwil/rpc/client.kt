...
open class ApiClient (
    ...
    private suspend inline fun <reified TRequest> doRequest(
        ...
        return httpClient.post("$baseUrl/rpc/v1") {
            ...
            headers {
                append(HttpHeaders.Cookie, requestCookie)
            }
            setBody(request)
        }
    }
    ...
    suspend fun health(): HealthResponse = 
        call(JSONRPCMethod.METHOD_HEALTH, HealthRequest())
    ...
    suspend fun getAccountClient(id: AccountId): AccountResponse =
        call(JSONRPCMethod.METHOD_ACCOUNT, AccountRequest(id, if (this.unconfirmedNonce) AccountStatus.PENDING else AccountStatus.LATEST))

    //
    // https://github.com/trufnetwork/kwil-js/blob/main/src/api_client/client.ts#L190
    //
    suspend fun broadcastClient(tx: TransactionBase64, sync: BroadcastSyncType? = null): BroadcastResponse {
        if (!tx.isSigned()) {
    ...
    suspend fun callMethod(msg: Message): CallResponse =
        call(JSONRPCMethod.METHOD_CALL, msg)
    ...
    suspend fun query(query: String, params: Map<String, kotlinx.serialization.json.JsonElement>): QueryResponse = 
        call(JSONRPCMethod.METHOD_QUERY, SelectQueryRequest(query, params))
    ...
    suspend fun authParam(): KGWAuthInfo =
        call(JSONRPCMethod.METHOD_KGW_PARAM, AuthParamRequest())

    suspend fun authn(nonce: String, sender: HexString, signature: Base64String, signatureType: SignatureType): String? {
        val response = this.doRequest(JSONRPCMethod.METHOD_KGW_AUTHN, AuthnRequest(nonce, sender, Signature(signature, signatureType)))
